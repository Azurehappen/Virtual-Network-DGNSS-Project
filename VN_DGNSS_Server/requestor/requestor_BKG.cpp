...
static constexpr const char *local_ip = "127.0.0.1";  // IP for BKG data output
static constexpr int EPH_port = 3536; // IP port for Eph data
static constexpr int SSR_port = 6699; // IP port for SSR data
static constexpr int reqst_interval =
    100000;  // the request SSR&EPH interval, 0.1s
static constexpr int SSRLOG_PERIOD =
    1800;                             // log period of IGS corr data in seconds
static constexpr int FILE_PERIOD =
    86400;  // log file update period in seconds (1 day)
static constexpr int CHECK_PERIOD =
    2*1000000;  // check IP port period in microseconds (2 s)
...
void requestor_BKG::clear_input_stream(std::stringstream &ss, std::string &line) {
  ss.clear();
  ss.str("");
  ss << line;
}
...
void requestor_BKG::parse_vtec(vtec_t &new_vtec, std::stringstream &ssr_ss, std::string line) {
    ...
    while (!ssr_ss.eof()) {
      ...
      if (line[0] == '>') {
        ...
        if (type == "CLOCK" || type == "ORBIT" || type == "VTEC" ||
          ...
          line_ss >> datetime[0] >> datetime[1] >> datetime[2]
...
void requestor_BKG::GPS_eph_parser(std::stringstream &eph_ss,
  ...
  double IODE;
  ss >> IODE >> eph_element.C_rs >> eph_element.Delta_n >>
     eph_element.M_0;
  ...
  eph_element.t_oe = adjweek(gpst2time(eph_element.weekNo,eph_element.toes),t_oc);
  // read line 7
  getline(eph_ss, line);
  clear_input_stream(ss, line);
...
void requestor_BKG::BDS_eph_parser(std::stringstream &eph_ss,
  ...
  clear_input_stream(ss, line);
  double AODE;
  ss >> AODE >> eph_element.C_rs >> eph_element.Delta_n >>
     eph_element.M_0;
  // read line 3
  getline(eph_ss, line);
  clear_input_stream(ss, line);
    ...
    while (!eph_ss.eof()) {
      ...
      if (line[0] == 'G' || line[0] == 'E' || line[0] == 'C') {
        ...
        if (line[0] == 'G') {
          ...
          // only reserve I/NAV massage (Date source: 517 for I/NAV, 258 for F/NAV)
        ...
        if (check == 0) { // When check = 1, nothing update
          num_sv++;
          log_eph << line[0] <<eph_element.prn<<" IODE "
                  << log_iod_old <<" updated to "<< log_iod_new <<std::endl;
          ...
          if (line[0] == 'G') {
            ...
            for (int ver = EPH_VERMAX-1;ver > 0;ver--) {
              eph_data[ver].GPS_eph[eph_element.prn] = eph_data[ver-1].GPS_eph[eph_element.prn];
            }
            eph_data[0].GPS_eph[eph_element.prn] = eph_element;
          } else if (line[0] == 'E') {
            std::lock_guard<std::mutex> lock(eph_mutex);
            for (int ver = EPH_VERMAX-1;ver > 0;ver--) {
              eph_data[ver].GAL_eph[eph_element.prn] = eph_data[ver-1].GAL_eph[eph_element.prn];
            }
            eph_data[0].GAL_eph[eph_element.prn] = eph_element;
          } else if (line[0] == 'C') {
            std::lock_guard<std::mutex> lock(eph_mutex);
            for (int ver = EPH_VERMAX-1;ver > 0;ver--) {
              eph_data[ver].BDS_eph[eph_element.prn] = eph_data[ver-1].BDS_eph[eph_element.prn];
            }
