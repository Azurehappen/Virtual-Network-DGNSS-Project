...
\begin{document}
	...
	Since the receivers were all connected to the same antenna, the effects on the experiment of the surroundings are the same for all receivers. 
	\begin{figure}[H]
		\centering
		\hfill
	...
	The receiver performing RTK did not have SBAS and Galileo satellites because u-blox M8P does not support them. 
	\black
	...
	Purple indicates satellites whose signal was available but not use in navigation for other reasons. 
	Red  indicates satellites whose signal is not available. 
	Several reasons will cause purple or red \cite{ucenter}, e.g.: unhealthy status reported by satellite, lack of ephemeris, low signal strength, below the elevation cutoff angle. 
	%
	The orbits for some satellites, such as B21, E5, and G24, change colors in Figs. \ref{fig:mt1} and Fig. \ref{fig:mt2}.
	...
	Fig. \ref{fig:m1vspeed}b and Fig. \ref{fig:m2vspeed}b show the vehicle speed over ground as computed using the RTK data. 
	
	Fig. \ref{fig:m1vspeed}c and Fig. \ref{fig:m2vspeed}c show the number of satellites used in the navigation solution for each of the experimental configurations listed  by acronym in Table IV of \cite{hu2021using}. 
	%
	...
	\todo{WH:
		...
		\begin{enumerate}
			\item See the marked up pdf.  Implement all changes.
			\item In tex documents, never hardcode a table or biblio entry. Use labels.  I had to fix two in this document.
			\item There are two moving experiments. Clearly state which experiment corresponds with Fig. \ref{fig:qi}	.
			\item The horizontal axis of Fig. \ref{fig:qi} needs to match the units of Figs \ref{fig:m1vspeed} and \ref{fig:m2vspeed}. One is in epochs while the other is in minutes. When I look at Fig. \ref{fig:qi}, I should be able to see the same number of satellites use in one of Figs \ref{fig:m1vspeed} and \ref{fig:m2vspeed}.
		\end{enumerate}
		I have made changes throughout. Please proofread carefully
	}
